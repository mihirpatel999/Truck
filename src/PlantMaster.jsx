...
const API_URL = import.meta.env.VITE_API_URL; // e.g., http://localhost:3001
...
export default function PlantMaster() {
  ...
  const fetchPlants = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/plantmaster`);
      setPlantList(res.data);
    } catch (err) {
      console.error('❌ Error fetching plant list:', err);
    }
  ...
  const handleEditClick = async (id) => {
    try {
      const res = await axios.get(`${API_URL}/api/plantmaster/${id}`);
      const plant = res.data;

      setPlantName(plant.PlantName || '');
      setPlantAddress(plant.PlantAddress || '');
      setContactPerson(plant.ContactPerson || '');
      setMobileNo(plant.MobileNo || '');
      setRemarks(plant.Remarks || '');
      setSelectedPlantId(id);
    } catch (err) {
      console.error('❌ Error fetching plant data:', err.response || err.message);
      alert(`Error fetching plant data: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedPlantId) {
        await axios.put(`${API_URL}/api/plantmaster/${selectedPlantId}`, newPlant);
        alert('Plant updated successfully');
      } else {
        await axios.post(`${API_URL}/api/plantmaster`, newPlant);
  ...
  return (
    <div className="p-4 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {selectedPlantId ? 'Update Plant' : 'Add Plant'}
        </button>
      </form>

      <hr className="my-6" />

      <h2 className="text-xl font-semibold mb-2">📋 Existing Plants</h2>
      <ul>
        {plantList.map((plant) => (
          <li key={plant.PlantID} className="flex justify-between items-center mb-2">
