...
export default function Home() {
  ...
  const allowedPanels = panelList
    .filter(p => {
      if (!userRole) return false;
      const roles = userRole.split(",").map(r => r.trim());
      return roles.some(r => p.roles.includes(r));
    })
    .sort((a, b) => (a.name === "Gate Keeper" ? -1 : b.name === "Gate Keeper" ? 1 : 0)); // Gate Keeper first

  // ✅ Desktop version
  if (!isMobile) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-purple-50 to-pink-100">
        <h1 className="text-4xl font-bold text-gray-800">Please open on a mobile device</h1>
      </div>
    );
  }
  ...
  // ✅ Mobile version
  return (
    <div className="min-h-screen bg-gradient-to-tr from-purple-50 to-pink-100 p-4 flex flex-col">
      <div className="flex items-center mb-6">
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/4/48/Emoji_u1f34b.svg"
          alt="Lemon Logo"
        ...
        />
        <h2 className="text-xl font-bold text-purple-800">Lemon ERP</h2>
      </div>

      <div className={`grid ${allowedPanels.length === 1 ? 'grid-cols-1' : 'grid-cols-2'} gap-4 flex-grow`}>
        {allowedPanels.map((p, idx) => (
          <Link
            to={p.path}
            key={idx}
            className="bg-gradient-to-br from-white to-purple-100 rounded-3xl shadow-xl p-5 flex flex-col items-center justify-center text-center hover:scale-105 transition-all border border-purple-200"
          >
      ...
      <button
        ...
        className="mt-6 bg-red-500 text-white py-2 rounded-xl shadow hover:bg-red-600"
      >
        Logout
      </button>
    </div>
