      ...
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-1 space-y-4">
          <div className="relative h-[200px] w-full bg-blue-200 rounded-lg overflow-hidden shadow-md">
            ...
            />
            <div className="absolute left-[6%] right-[24%] bottom-[37%] top-[12%] z-10 flex justify-between items-end">
              {quantityPanels.map((panel, index) => {
                const height = maxQty ? (panel.quantity / maxQty) * 100 : 0;
                ...
                return (
                  <div
                    key={index}
                    className={`flex flex-col items-center justify-end text-white text-[10px] ${bgColors[index % bgColors.length]} rounded-t-md hover:scale-105 transition-all duration-150`}
                    style={{ height: `${height}%`, width: `${100 / quantityPanels.length - 1}%`, minWidth: '30px' }}
                  >
                    <div>{panel.quantity}</div>
                    <div className="whitespace-nowrap text-[8px]">{panel.plantname}</div>
                  </div>
