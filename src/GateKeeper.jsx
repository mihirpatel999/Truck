...
// ✅ GateKeeper.jsx (Updated as per your new logic)
import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
...
function GateKeeper() {
  ...
  const handleTruckSelect = async (truckNo) => {
    ...
    try {
      ...
      const quantityRes = await axios.get(`${API_URL}/api/truck-plant-quantities?truckNo=${truckNo}`);
      const sortedQuantities = [...quantityRes.data].sort((a, b) => a.priority - b.priority);
      setQuantityPanels(sortedQuantities);

      // get FROM and NEXT
      const priorityRes = await axios.get(`${API_URL}/api/check-priority-status`, {
        params: { truckNo, plantName: selectedPlant }
      });
      const { nextPlant: next, hasPending } = priorityRes.data;

      const completedPlants = sortedQuantities.filter(q => q.quantity === 0).map(q => q.plantname);
      setFromPlant(completedPlants[completedPlants.length - 1] || 'Unknown');
      setNextPlant(hasPending ? next : 'Completed');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-indigo-100 p-6">
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-2xl p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          <div className="relative h-56 w-full bg-blue-200 rounded-lg overflow-hidden shadow-md">
            <div className="absolute bottom-[51px] left-[20px] h-[75px] w-[calc(100%-140px)] max-w-[400px] flex items-end gap-[2px] z-10">
              {quantityPanels.map((panel, index) => {
                const height = maxQty ? (panel.quantity / maxQty) * 100 : 0;
                const bgColors = ['bg-green-500', 'bg-blue-500', 'bg-yellow-500', 'bg-red-500'];
                return (
          ...
          <div className="flex justify-between text-center">
            <div className="bg-white border px-3 py-1 rounded w-[48%] text-sm">
              <strong>From:</strong> {fromPlant || 'Unknown'}
            </div>
            <div className="bg-white border px-3 py-1 rounded w-[48%] text-sm">
              <strong>Next:</strong> {nextPlant || 'Unknown'}
            </div>
          </div>
