    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-100 to-blue-200 p-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl p-4 shadow space-y-4">
          <select value={selectedPlant} onChange={handlePlantChange} className="w-full px-4 py-2 rounded border shadow-sm">
            <option value="">Select Plant</option>
            {plantList.map((plant, idx) => (
              <option key={idx} value={getPlantName(plant)}>{getPlantName(plant)}</option>
            ))}
          </select>
          <div className="bg-blue-50 rounded-lg p-4 h-[300px] overflow-y-auto">
            <h3 className="font-semibold mb-2 text-blue-800">Truck List</h3>
            <ul className="space-y-1 text-sm">
              {truckNumbers.map((truck, i) => (
                <li key={i} onClick={() => handleTruckSelect(getTruckNo(truck))} className="hover:text-blue-600 cursor-pointer">
                  {getTruckNo(truck)}
                </li>
              ))}
              {truckNumbers.length === 0 && <li className="text-gray-400 italic">No trucks available</li>}
        ...
        {/* Middle Panel with Truck + Chart */}
        <div className="bg-white rounded-xl p-4 shadow space-y-4">
          <div className="relative w-full h-[180px] bg-blue-100 rounded-lg overflow-hidden">
            <div className="absolute bottom-[58px] left-[8%] right-[8%] z-10 flex justify-between gap-2">
              {quantityPanels.map((panel, idx) => (
                <div key={idx} className="bg-green-500 text-white text-xs font-bold rounded-full px-3 py-1 shadow text-center flex-1 whitespace-nowrap">
                  <div>{panel.quantity}</div>
                  <div className="text-[10px] font-normal">{panel.plantname}</div>
                </div>
              ))}
            </div>
            <img
          ...
          {/* Form */}
          <div className="space-y-2">
            <input name="truckNo" value={formData.truckNo} onChange={handleChange} className="w-full border px-4 py-2 rounded shadow-sm" placeholder="Truck No" />
            <input type="date" name="dispatchDate" value={formData.dispatchDate} onChange={handleChange} className="w-full border px-4 py-2 rounded shadow-sm" />
            <input name="invoiceNo" value={formData.invoiceNo} onChange={handleChange} className="w-full border px-4 py-2 rounded shadow-sm" placeholder="Invoice No" />
            <textarea name="remarks" readOnly value={formData.remarks} className="w-full h-20 resize-none bg-gray-100 border px-4 py-2 rounded shadow-sm text-sm" />
          </div>

          <div className="flex gap-2 pt-2">
            <button className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded shadow" onClick={() => handleSubmit('Check In')}>✅ Check In</button>
            <button className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded shadow" onClick={() => handleSubmit('Check Out')}>❌ Check Out</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow">
          ...
          <div className="bg-green-50 rounded-lg p-4 h-[300px] overflow-y-auto">
            <ul className="space-y-1 text-sm text-gray-700">
              {checkedInTrucks.map((truck, i) => (
                <li key={i} onClick={() => handleTruckSelect(getTruckNo(truck))} className="hover:text-green-600 cursor-pointer">
                  {getTruckNo(truck)}
                </li>
              ))}
              {checkedInTrucks.length === 0 && (
