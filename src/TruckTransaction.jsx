...
export default function TruckTransaction() {
  ...
  const [tableData, setTableData] = useState([
    { plantName: "", loadingSlipNo: "", qty: "", priority: 1, remarks: "", freight: "" },
  ]);
  ...
  useEffect(() => {
    if (editData) {
      setFormData({
        ...
        amountPerTon: editData.amountperton,
        truckWeight: editData.truckweight,
        deliverPoint: editData.deliverpoint,
        remarks: editData.remarks,
        transactionId: editData.transactionid,
      });

      setTableData(editData.details.map(d => ({
        plantName: d.plantname,
        loadingSlipNo: d.loadingslipno,
        qty: d.qty,
        priority: d.priority,
        remarks: d.remarks,
        freight: d.freight
  ...
  const addRow = () => {
    setTableData(prev => [...prev, { plantName: "", loadingSlipNo: "", qty: "", priority: "", remarks: "", freight: "" }]);
  };
  ...
  const validate = () => {
    const plantNames = tableData.map(row => row.plantName.trim().toLowerCase());
    const uniquePlantNames = [...new Set(plantNames)];

    if (plantNames.length !== uniquePlantNames.length) {
      toast.error("🚫 Duplicate plant names are not allowed.");
      return false;
    }

    const hasPriorityOne = tableData.some(row => Number(row.priority) === 1);
    if (!hasPriorityOne) {
      toast.error("🚫 At least one plant must have Priority 1.");
      return false;
    }
  ...
  const handleSubmit = async () => {
    if (!formData.truckNo) return toast.error("Please enter Truck No.");
    if (!validate()) return;
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setTimeout(() => navigate("/truck-schedule"), 1000);
      } else {
      ...
      console.error(err);
      if (err.response?.status === 409) {
        toast.error("🚫 Truck is already in transport. Complete check-out first.");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-indigo-100 p-6">
      <div className="max-w-5xl mx-auto bg-white shadow-2xl rounded-3xl p-6 space-y-6">
        <h2 className="text-2xl font-bold text-indigo-700">🚚 Truck Transaction</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input name="truckNo" value={formData.truckNo} onChange={handleChange} placeholder="Truck No" className="border p-2 rounded" />
          <input name="transactionDate" type="date" value={formData.transactionDate} onChange={handleChange} className="border p-2 rounded" />
          <input name="cityName" value={formData.cityName} onChange={handleChange} placeholder="City" className="border p-2 rounded" />
          ...
          <input name="deliverPoint" value={formData.deliverPoint} onChange={handleChange} placeholder="Deliver Point" className="border p-2 rounded" />
          <input name="remarks" value={formData.remarks} onChange={handleChange} placeholder="Remarks" className="border p-2 rounded col-span-3" />
        </div>

        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-indigo-600">Plant Details</h3>

          {tableData.map((row, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-6 gap-2 items-center">
              <input name="plantName" value={row.plantName} onChange={(e) => handleTableChange(index, e)} placeholder="Plant Name" className="border p-2 rounded" />
              <input name="loadingSlipNo" value={row.loadingSlipNo} onChange={(e) => handleTableChange(index, e)} placeholder="Loading Slip No" className="border p-2 rounded" />
              <input name="qty" value={row.qty} onChange={(e) => handleTableChange(index, e)} placeholder="Qty" className="border p-2 rounded" />
              <input name="priority" value={row.priority} onChange={(e) => handleTableChange(index, e)} placeholder="Priority" className="border p-2 rounded" />
              <input name="freight" value={row.freight} onChange={(e) => handleTableChange(index, e)} placeholder="Freight" className="border p-2 rounded" />
              <button onClick={() => removeRow(index)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Remove</button>
            </div>
          ))}

          <button onClick={addRow} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">+ Add Plant</button>
        </div>

        <div className="flex justify-end">
          <button onClick={handleSubmit} className="bg-indigo-700 text-white px-6 py-2 rounded hover:bg-indigo-800">Save Transaction</button>
        </div>
      </div>
