...
export default function TruckTransaction() {
  ...
  const addOrUpdateRow = () => {
    ...
    // Priority duplicate validation
    const existingPriorities = tableData
      .filter((_, idx) => idx !== editingIndex)
      .map(row => row.priority);

    if (newRow.priority) {
      if (existingPriorities.includes(newRow.priority)) {
        alert(`Priority ${newRow.priority} is already assigned.`);
        return;
      }

      if (newRow.priority === '1' && existingPriorities.includes('1')) {
        alert("Priority 1 is already assigned. You cannot assign it again.");
        return;
      }
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Final duplicate priority validation before submit
      ...
      const hasMultiplePriorityOne = priorities.filter(p => p === '1').length > 1;

      if (hasDuplicatePriority || hasMultiplePriorityOne) {
        alert("Duplicate Priority or multiple Priority 1 not allowed.");
        return;
      }
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 md:p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label className="font-medium text-slate-700 mb-1 block">Truck No</label>
            <input
              type="text" name="truckNo" maxLength={13}
