...
import { FaListAlt } from 'react-icons/fa';  // For Plant List Icon
  ...
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { title: "User Management", path: "/usermaster", icon: <FiUsers size={16} /> },
        { title: "User Register", path: "/userregister", icon: <BsShieldLock size={16} /> },
        { title: "Plant List", path: "/plantlist", icon: <FaListAlt size={16} /> }  // Added Plant List
      ]
    },
    {
      title: "Dispatch",
            ...
            <div className="flex items-center space-x-4">
              <div className="flex space-x-1">
                {filteredMenuItems.map((item, index) => (
                  <div key={index} className="relative h-full">
                    {item.path ? (
                      <Link to={item.path} className={`flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${location.pathname === item.path ? 'text-blue-700 bg-blue-50 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                        <span className="mr-2">{item.icon}</span>
                        {item.title}
                      </Link>
                    ) : (
                      <div className="h-full">
                        <button onClick={(e) => { e.stopPropagation(); setActiveDropdown(activeDropdown === index ? null : index); }} className={`flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${activeDropdown === index || item.subItems?.some(sub => location.pathname === sub.path) ? 'text-blue-700 bg-blue-50 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                          <span className="mr-2">{item.icon}</span>
                          {item.title}
                          <FiChevronDown className={`ml-1 h-4 w-4 transition-transform duration-200 ${activeDropdown === index ? 'rotate-180' : ''}`} />
                        </button>

                        {activeDropdown === index && (
                          <div className="absolute right-0 mt-1 w-56 bg-white rounded-lg shadow-lg ring-1 ring-gray-200 py-1 z-50" onClick={(e) => e.stopPropagation()}>
                            {item.subItems.map((subItem, subIndex) => (
                              <Link key={subIndex} to={subItem.path} onClick={() => setActiveDropdown(null)} className={`flex items-center px-4 py-2.5 text-sm transition-colors ${location.pathname === subItem.path ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
                                <span className="mr-3 text-gray-500">{subItem.icon}</span>
                                {subItem.title}
                              </Link>
                            ))}
              ...
              <button onClick={handleLogout} className="flex items-center px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors min-w-max">
                <FiLogOut className="mr-2" />
                Logout
              </button>
            </div>
      ...
      <nav className="md:hidden bg-white shadow-sm sticky top-0 z-50 border-b border-gray-100">
        <div className="px-4">
          <div className="flex justify-between h-16 items-center">
            ...
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="text-gray-500 hover:text-gray-700 focus:outline-none p-2 rounded-full hover:bg-gray-100 transition-colors">
        ...
        <div className={`fixed inset-0 z-40 transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
          ...
          <div className="relative flex flex-col w-80 max-w-sm h-full bg-white shadow-xl">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200">
              <div className="text-xl font-semibold text-gray-800">Menu</div>
              <button onClick={() => setMobileMenuOpen(false)} className="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100">
                <FiX className="h-6 w-6" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto py-4">
              {filteredMenuItems.map((item, index) => (
                <div key={index} className="px-2">
                  {item.path ? (
                    <Link to={item.path} onClick={() => setMobileMenuOpen(false)} className={`flex items-center px-4 py-3 rounded-lg mx-2 text-base font-medium ${location.pathname === item.path ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>
                      <span className="mr-3">{item.icon}</span>
                      {item.title}
                    </Link>
                  ) : (
                    <div>
                      <button onClick={(e) => { e.stopPropagation(); setActiveDropdown(activeDropdown === index ? null : index); }} className={`flex items-center justify-between w-full px-4 py-3 rounded-lg mx-2 text-base font-medium ${activeDropdown === index ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>
                        <div className="flex items-center">
                          <span className="mr-3">{item.icon}</span>
                          {item.title}
                        </div>
                        <FiChevronDown className={`h-5 w-5 transition-transform ${activeDropdown === index ? 'rotate-180' : ''}`} />
                      </button>

                      <div className={`overflow-hidden transition-all duration-300 ${activeDropdown === index ? 'max-h-96' : 'max-h-0'}`}>
                        {item.subItems.map((subItem, subIndex) => (
                          <Link key={subIndex} to={subItem.path} onClick={() => { setMobileMenuOpen(false); setActiveDropdown(null); }} className={`flex items-center pl-12 pr-4 py-2.5 text-base ${location.pathname === subItem.path ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}>
                            <span className="mr-3">{subItem.icon}</span>
                            {subItem.title}
                          </Link>
                        ))}
            ...
            <div className="px-4 py-4 border-t border-gray-200">
              <button onClick={handleLogout} className="flex items-center justify-center w-full px-4 py-3 rounded-lg bg-gray-50 text-red-600 hover:bg-red-50 font-medium">
                <FiLogOut className="mr-3" />
                Logout
              </button>
            </div>
