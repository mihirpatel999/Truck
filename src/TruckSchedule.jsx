  ...
  return (
    <div className="p-4 bg-gradient-to-br from-gray-100 to-blue-100 min-h-screen flex flex-col items-center w-full">
      ...
      <div className="flex flex-wrap gap-3 justify-center items-end bg-white p-3 rounded-lg shadow w-full max-w-5xl mb-4">
        ...
        <div className="flex gap-1 flex-wrap">
          {['Dispatched', 'InTransit', 'CheckedOut', 'All'].map(btn => (
            <button key={btn} onClick={() => fetchData(btn, truckSearch)} className={`px-3 py-1 rounded font-semibold text-xs ${status === btn ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700'}`}>{btn}</button>
          ))}
        </div>
        <input type="text" placeholder="Truck No." value={truckSearch} onChange={e => setTruckSearch(e.target.value)} className="border rounded px-2 py-1 text-sm w-24" />
        <button onClick={() => fetchData(status, truckSearch)} className="px-3 py-1 bg-indigo-500 text-white rounded text-sm">Search</button>
      </div>

      <div className="bg-white p-3 rounded-lg shadow w-full max-w-5xl mb-4">
        <h2 className="text-base font-semibold mb-2">Select Plants</h2>
        <div className="flex gap-2 mb-2 flex-wrap">
          <button onClick={selectAll} className="bg-green-500 text-white px-3 py-1 rounded text-sm">Select All</button>
          <button onClick={deselectAll} className="bg-red-500 text-white px-3 py-1 rounded text-sm">Deselect</button>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 max-h-40 overflow-y-auto">
          {plantList.map(p => (
            <label key={p.plantid} className="flex items-center gap-1 text-sm">
      ...
      <div className="bg-white p-3 shadow rounded-2xl w-full max-w-5xl overflow-x-auto">
        ...
        {loading && <p className="text-indigo-600 text-center">Loading...</p>}
        {error && <p className="text-red-500 text-center">{error}</p>}
        {!loading && !error && data.length === 0 && (<p className="text-gray-500 text-center">No trucks found</p>)}
              ...
              <tbody>
                {data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    {['truckNo', 'plantName', 'checkInTime', 'checkOutTime', 'loadingSlipNo', 'qty', 'freight', 'priority'].map((key) => {
                      const raw = item[key];
                      const value = key.includes('Time') && raw ? new Date(raw).toLocaleString() : raw ?? '—';
                      return (<td key={key} className="px-3 py-2 border border-gray-300">{value}</td>);
                    })}
                  </tr>
                ))}
              </tbody>
            ...
            {data.map((item, idx) => (
              <div key={idx} className="border border-gray-300 rounded p-2 shadow-sm bg-white">
                {['Truck No', 'Plant', 'Check‑In', 'Check‑Out', 'Slip', 'Qty', 'Freight', 'Priority'].map((label, i) => {
                  ...
                  const raw = item[key];
                  const value = key.includes('Time') && raw ? new Date(raw).toLocaleString() : raw ?? '—';
                  return (<p key={key} className="text-sm"><span className="font-semibold">{label}:</span> {value}</p>);
                })}
              </div>
            ))}
          </div>
