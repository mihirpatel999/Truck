...
function TruckFind() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/truck-find`);
        setTruckData(res.data);
      } catch (err) {
        console.error('Error fetching truck list:', err);
  ...
  const handleEdit = async (truckNo) => {
    try {
      const res = await axios.get(`${API_URL}/api/truck-transaction/${encodeURIComponent(truckNo)}`);
      const truckDetails = res.data;
  ...
  return (
    <div className="truck-find-container">
      <h1>Search Result</h1>
      {error && <p className="error-text">{error}</p>}
      <table className="truck-find-table">
        <thead>
          <tr>
        ...
        <tbody>
          {truckData.map((truck, idx) => (
            <tr key={idx}>
              <td>{new Date(truck.transactiondate).toLocaleDateString()}</td>
              <td>{truck.cityname}</td>
              <td>
                <button className="btn btn-warning" onClick={() => handleEdit(truck.truckno)}>
                  Edit
                </button>
              </td>
            </tr>
